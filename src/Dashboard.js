...
function Dashboard({ isDarkMode, setIsDarkMode }) {
  ...
  const [currency, setCurrency] = useState("USD"); // REMOVED alerts state
  ...
  const handleLogout = () => {
    ...
    localStorage.clear(); setTransactions([]); window.location.href = "/login";
  };
  ...
  const categoryOptions = {
    ...
    plugins: {
      ...
      tooltip: { backgroundColor: isDarkMode? '#1F2937' : '#FFFFFF', titleColor: textColor, bodyColor: textColor, borderColor: gridColor, borderWidth: 2, callbacks: { label: (context) => `${context.label}: ${formatAmount(context.parsed, currency)}` } }
    }
  };
  ...
  const profitOptions = {
    ...
    plugins: {
      ...
      tooltip: { backgroundColor: isDarkMode? '#1F2937' : '#FFFFFF', titleColor: textColor, bodyColor: textColor, borderColor: gridColor, borderWidth: 2, callbacks: { label: (context) => `Net Profit: ${formatAmount(context.parsed.y, currency)}` } }
    },
    scales: {
      y: { grid: { color: gridColor }, ticks: { color: textColor, font: { weight: '600', size: 13 }, callback: (value) => formatAmount(value, currency) } },
      x: { grid: { color: gridColor }, ticks: { color: textColor, font: { weight: '600', size: 13 }, maxRotation: 45, minRotation: 45 } }
    }
  };
    ...
    <>
      <div id="dashboard-to-pdf" className={isDarkMode? "bg-gray-900 text-white min-h-screen p-6" : "bg-gray-100 text-gray-900 min-h-screen p-6"}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
          <div onClick={() => filterByType('income')} className="cursor-pointer bg-gradient-to-br from-green-500 to-emerald-600 p-6 rounded-xl shadow-xl text-white hover:scale-105 transition"><div className="flex justify-between items-center"><h3 className="text-xl font-bold">Total Revenue</h3><FaArrowUp className="text-white text-2xl" /></div><p className="text-3xl font-extrabold mt-2">{formatAmount(totalRevenue, currency)}</p></div>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-amber-600 p-6 rounded-xl shadow-xl text-white"><div className="flex justify-between items-center"><h3 className="text-xl font-bold">Runway</h3><FaBalanceScale className="text-white text-2xl" /></div><p className="text-3xl font-extrabold mt-2">{calcRunway}</p><p className="text-xs mt-1 opacity-80">Based on last 30 days</p></div>
        </div>
        ...
        <Alerts dashboardData={{ totalRevenue, totalExpenses, netProfit, profitMargin }} />
        ...
        <div id="transactions-table" className={isDarkMode? "bg-gray-800 p-6 rounded-xl shadow-lg text-white" : "bg-white p-6 rounded-xl shadow-lg text-gray-900"}>
          ...
          <div className="flex gap-3 mb-4 flex-wrap">
            ...
            <div className="date-wrapper"><input type="date" value={dateRange.start} onChange={(e) => { setDateRange({...dateRange, start: e.target.value }); setCurrentPage(1); }} className={isDarkMode? "border-2 border-gray-600 rounded-lg px-3 py-2 pr-10 bg-gray-700 text-white" : "border-2 border-gray-300 rounded-lg px-3 py-2 pr-10 bg-white text-gray-900"} /></div>
            <div className="date-wrapper"><input type="date" value={dateRange.end} onChange={(e) => { setDateRange({...dateRange, end: e.target.value }); setCurrentPage(1); }} className={isDarkMode? "border-2 border-gray-600 rounded-lg px-3 py-2 pr-10 bg-gray-700 text-white" : "border-2 border-gray-300 rounded-lg px-3 py-2 pr-10 bg-white text-gray-900"} /></div>
          </div>
